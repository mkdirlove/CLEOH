#!/usr/bin/python
'''
<Simple Fb Chat Bot by Jayson Cabrillas San Buenaventura.
	...
	def onMessage(self,author_id,message_object,thread_id=None,thread_type=ThreadType.USER,**kwargs):
		...
		if author_id!=self.uid:
			if Main_Client.bool==False:
				...
				self.messge_send=self.send(Message(text='''How Can I Help You Ma'am / Sir? My name is Cleoh (Facebook Chat Bot) created by Master Jayson San Buenaventura,\nNeed some help??\nJust type the command ✒️ qoutes, Help or help.'''),thread_id=thread_id,thread_type=thread_type)
			if 'change colour to' in self.msg:
				...
				self.messge_send=self.send(Message(text='''Sorry Ma'am / Sir!!! I Don't understand.😞'''),thread_id=thread_id,thread_type=thread_type)
